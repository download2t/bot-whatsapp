...
Cada `User` do painel tem **seu próprio WhatsApp conectado, seus próprios `Contato`/`Turma`/`ScheduleRule`/`MessageLog`**, sem nenhuma visibilidade cruzada — **nem Admin vê os dados de WhatsApp de outro usuário**. Isso é reforçado em toda a stack:

- **Banco**: `Contato`, `Turma`, `ScheduleRule`, `MessageLog` têm `OwnerUserId`. Todo controller filtra/seta esse campo via `this.GetCurrentUserId()` (extension method em `Utils/ControllerExtensions.cs`, lê a claim `NameIdentifier` do JWT). Sem exceção para Admin.
...
5. **Mensagem por dia da semana**: uma `ScheduleRule` pode ter várias mensagens (`MessagesJson`, lista de `{ text, days[] }`) — a mensagem usada é a que está vinculada ao dia da semana atual. Se nenhuma mensagem cobre o dia, não responde (mesmo com regra ativa).
...
- **`ScheduleRule`** — regra de auto-resposta do dono (`OwnerUserId`): janelas de horário (`ScheduleWindowsJson`) + mensagens por dia da semana (`MessagesJson`). Não tem mais número WhatsApp associado — é sempre o número conectado do próprio dono.
...
- `POST /api/messages/bulk` — disparo em massa (por `TurmaId` ou lista de `ContactIds`, escopados ao dono), com streaming de progresso via NDJSON (`streamUpdates: true`).
- `POST /api/messages/bulk/send` — envio individual (usado pelo chat em `/messages`).
- Ambos aceitam `mediaBase64` + `mediaMimeType` + `mediaFileName` opcionais (imagem, vídeo ou documento — limite de 20MB) além do texto. O frontend converte o arquivo para base64 no browser (`components/MediaAttachment.tsx`) e envia dentro do mesmo corpo JSON — não é upload multipart.
...
- **Produção (VPS)**: o banco de produção foi construído historicamente via `EnsureCreated()` + SQL bruto ad-hoc (não via `dotnet ef database update`), então a tabela `__EFMigrationsHistory` de lá está vazia mesmo já tendo o schema das primeiras 3 migrations. Antes de rodar `dotnet ef database update` em produção pela primeira vez depois dessa mudança, é necessário "baselinar" o histórico (marcar as 3 migrations antigas como já aplicadas, sem executá-las) para que só as migrations novas rodem de verdade. Sempre faça backup do `app.db` antes. **Procedimento exato, com os `MigrationId` reais e os comandos SQL, está em `passo_deploy.md` seção 16.**
